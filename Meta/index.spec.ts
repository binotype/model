...
describe("binotype.Meta", () => {
	const validMeta: binotype.Meta[] = [
		...
		}, // Mixed value types
		{
			nested: {
				title: "Nested Meta",
				description: "A nested meta object"
			}
		}, // Nested Meta object
		{
			complex: {
	...
	it.each(validMeta)("is(%s)", value => 
		expect(binotype.Meta.is(value)).toBe(true)
	)

	it.each(validMeta)("flawed(%s)", value => 
		expect(binotype.Meta.flawed(value)).toBe(false)
	)
	...
	it.each(invalidMeta)("is(%s) should be false for invalid data", value => 
		expect(binotype.Meta.is(value)).toBe(false)
	)
